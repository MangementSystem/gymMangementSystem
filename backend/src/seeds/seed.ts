...
async function seed() {
  ...
  const workoutProgramExerciseRepo = dataSource.getRepository(
    WorkoutProgramExercise,
  );
  ...
  const memberships =
    (await membershipRepo.count()) > 0
      ? await membershipRepo.find({
          relations: ['organization', 'member', 'plan'],
        })
  ...
  const workoutProgramExercisesCount = await workoutProgramExerciseRepo.count();
  if (workoutProgramExercisesCount === 0) {
          ...
          workoutLogRepo.create([
            {
              ...
              date: new Date('2024-10-01'),
              ai_summary:
                'Strong hip drive and stable posture throughout sets.',
            },
            {
              member: members[1],
              program: workoutPrograms[1],
      ...
      aiExerciseAnalysisRepo.create([
        {
          ...
          risk_level: 'low',
          detected_errors: [
            { set: 3, issue: 'Knee valgus', severity: 'moderate' },
          ],
          recommended_fix:
            'Cue "knees out" and use resistance band for warm-up.',
        },
        {
          member: members[1],
          workout_log: workoutLogs[1],
          ...
          risk_level: 'medium',
          detected_errors: [
            { interval: '500m', issue: 'Rounded shoulders', severity: 'low' },
          ],
          recommended_fix:
            'Focus on keeping chest lifted, incorporate thoracic mobility drills.',
        },
        {
          member: members[2],
          workout_log: workoutLogs[2],
      ...
      aiInsightRepo.create([
        {
          ...
          ai_recommendation:
            'Increase accessory work for posterior chain twice per week.',
        ...
        },
        {
          ...
          ai_recommendation:
            'Add one extra low-intensity steady state session weekly.',
        ...
        },
        {
          ...
          input_data: {
            shoulder_flexion: 'Improved',
            hamstring_test: 'Tightness',
          },
